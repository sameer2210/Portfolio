...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  // Memoize default props to prevent recreations
  ...
  const validateData = useCallback((data: Position[]): boolean => {
  ...
  // Memoize material building function
  ...
  // Memoize data building function
  const buildData = useCallback(() => {
    ...
    for (let i = 0; i < arcs.length; i++) {
      ...
      const colorFunction = (t: number) => `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${1 - t})`;
  ...
  }, [data, validateData, defaultProps.pointSize]);

  // Memoize animation start function
  const startAnimation = useCallback(() => {
    if (!globeRef.current || !globeData || !validateData(data)) return;
...
export function World(props: WorldProps) {
  ...
  return (
    <Canvas scene={scene} camera={camera}>
      ...
      />
      <pointLight color={globeConfig.pointLight} position={lightPositions.point} intensity={0.8} />
      <Globe {...props} />
      <OrbitControls
        enablePan={false}
        enableZoom={false}
